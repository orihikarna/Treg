...
vtkStandardNewMacro(vtkImageEuclideanDistance2);

//------------------------------------------------------------------------------
// This defines the default values for the EDT parameters
vtkImageEuclideanDistance2::vtkImageEuclideanDistance2() {
...
int vtkImageEuclideanDistance2::IterativeRequestInformation(vtkInformation* vtkNotUsed(input), vtkInformation* output) {
...
int vtkImageEuclideanDistance2::IterativeRequestUpdateExtent(vtkInformation* input, vtkInformation* vtkNotUsed(output)) {
...
// is always doubles.
template <class TT>
void vtkImageEuclideanDistanceCopyData(vtkImageEuclideanDistance2* self, vtkImageData* inData, TT* inPtr, vtkImageData* outData, int outExt[6],
                                       DistType* outPtr) {
  ...
  DistType *outPtr0, *outPtr1, *outPtr2;
...
// is always doubles.
template <class T>
void vtkImageEuclideanDistanceInitialize(vtkImageEuclideanDistance2* self, vtkImageData* inData, T* inPtr, vtkImageData* outData, int outExt[6],
                                         DistType* outPtr) {
  vtkIdType inInc0, inInc1, inInc2;
  ...
  {
    vtkImageEuclideanDistanceCopyData(self, inData, static_cast<T*>(inPtr), outData, outExt, static_cast<DistType*>(outPtr));
  }
}
...
//
static void vtkImageEuclideanDistanceExecuteSaito(vtkImageEuclideanDistance2* self, vtkImageData* outData, int outExt[6], DistType* outPtr) {
  int outMin0, outMax0, outMin1, outMax1, outMin2, outMax2;
  vtkIdType outInc0, outInc1, outInc2;
  DistType *outPtr0, *outPtr1, *outPtr2;
  ...
  buff = static_cast<DistType*>(calloc(outMax0 + 1, sizeof(DistType)));

  // precompute sq[]. Anisotropy is handled here by using Spacing information
  sq = static_cast<DistType*>(calloc(inSize0 * 2 + 2, sizeof(DistType)));
  ...
  if (self->GetConsiderAnisotropy()) {
    ...
    std::cout << "Iteration = " << self->GetIteration() << ", spacing = " << spacing << std::endl;
  } else {
    spacing = 1;
...
//
static void vtkImageEuclideanDistanceExecuteSaitoCached(vtkImageEuclideanDistance2* self, vtkImageData* outData, int outExt[6], DistType* outPtr) {
  int outMin0, outMax0, outMin1, outMax1, outMin2, outMax2;
  vtkIdType outInc0, outInc1, outInc2;
  DistType *outPtr0, *outPtr1, *outPtr2;
  //
  ...
  buff = static_cast<DistType*>(calloc(outMax0 + 1, sizeof(DistType)));
  temp = static_cast<DistType*>(calloc(outMax0 + 1, sizeof(DistType)));
...
//------------------------------------------------------------------------------
void vtkImageEuclideanDistance2::AllocateOutputScalars(vtkImageData* outData, int outExt[6], vtkInformation* outInfo) {
  outData->SetExtent(outExt);
  outData->AllocateScalars(outInfo);
  std::cout << __FUNCTION__ << ", " << outData->GetScalarType() << std::endl;
}
...
int vtkImageEuclideanDistance2::IterativeRequestData(vtkInformation* vtkNotUsed(request), vtkInformationVector** inputVector,
  ...
  inPtr = inData->GetScalarPointerForExtent(inInfo->Get(vtkStreamingDemandDrivenPipeline::UPDATE_EXTENT()));
  ...
  if (outData->GetScalarType() != VTK_FLOAT) {
    vtkErrorMacro(<< "Execute: Output must be type flaot.");
    return 1;
  }
  ...
  if (this->GetIteration() == 0) {
    switch (inData->GetScalarType()) {
      vtkTemplateMacro(vtkImageEuclideanDistanceInitialize(this, inData, static_cast<VTK_TT*>(inPtr), outData, outExt, static_cast<DistType*>(outPtr)));
      default:
        vtkErrorMacro(<< "Execute: Unknown ScalarType");
        return 1;
    }
  } else {
    if (inData != outData) switch (inData->GetScalarType()) {
        vtkTemplateMacro(vtkImageEuclideanDistanceCopyData(this, inData, static_cast<VTK_TT*>(inPtr), outData, outExt, static_cast<DistType*>(outPtr)));
      }
  }
  ...
  switch (this->GetAlgorithm()) {
    case VTK_EDT_SAITO:
      vtkImageEuclideanDistanceExecuteSaito(this, outData, outExt, static_cast<DistType*>(outPtr));
      break;
    case VTK_EDT_SAITO_CACHED:
      vtkImageEuclideanDistanceExecuteSaitoCached(this, outData, outExt, static_cast<DistType*>(outPtr));
      break;
    default:
...
//------------------------------------------------------------------------------
void vtkImageEuclideanDistance2::PrintSelf(ostream& os, vtkIndent indent) {
